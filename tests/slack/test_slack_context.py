...
def test_strip_bot_mention_removes_bot_username_tag() -> None:
    assert (
        strip_bot_mention("@alephat please check", "UBOT", bot_username="alephat") == "please check"
    )


def test_replace_bot_mention_with_username() -> None:
    assert (
        replace_bot_mention_with_username("<@UBOT> can you help?", "UBOT", "alephat")
        == "@alephat can you help?"
    )
...
def test_parse_github_pr_url_raw_url() -> None:
    pr_ref = parse_github_pr_url("https://github.com/moule3053/alephat/pull/1244")

    assert pr_ref is not None
    assert pr_ref.owner == "moule3053"
    assert pr_ref.repo == "alephat"
    assert pr_ref.number == 1244
    assert pr_ref.url == "https://github.com/moule3053/alephat/pull/1244"


def test_parse_github_pr_url_slack_formatted_link() -> None:
    pr_ref = parse_github_pr_url("<https://github.com/moule3053/alephat/pull/1244|PR>")
...
def test_format_slack_messages_for_prompt_replaces_bot_id_mention_in_text() -> None:
    ...
    assert formatted == "@alice(U123): @alephat status update?"
...
def test_select_slack_context_messages_detects_username_mention() -> None:
    selected, mode = select_slack_context_messages(
        [
            {"ts": "1.0", "text": "@alephat first request", "user": "U1"},
            {"ts": "2.0", "text": "follow up", "user": "U2"},
            {"ts": "3.0", "text": "@alephat second request", "user": "U3"},
        ],
        "3.0",
        bot_user_id="UBOT",
        bot_username="alephat",
...
def test_process_slack_mention_creates_thread_first_run_without_trace_reply(
    ...
    assert captured["metadata_update"] == {
        ...
        "metadata": {"repo": {"owner": "moule3053", "name": "alephat"}},
    }
    ...
    assert "## Default Repository Hint\nmoule3053/alephat" in prompt_block["text"]
    assert (
    ...
    assert "## Alephat Links" in prompt_block["text"]
